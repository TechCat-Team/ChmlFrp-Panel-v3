<template>
            ...
            <n-tabs type="line" size="large" @update:value="handleTabChange">
                <n-tab-pane name="自定义解析">
                    <n-form style="margin-top: 10px;" :model="model" ref="formRef" label-placement="left"
                        label-width="auto" require-mark-placement="right-hanging" :size="size" :rules="rules">
                        ...
                        <div style="display: flex; justify-content: flex-end">
                            <n-button round type="primary" :loading="determineLoading" @click="handleSubmit"
                ...
                <n-tab-pane name="快速解析">
                    <n-form style="margin-top: 10px;" :model="fastModel" ref="formRefFast" label-placement="left"
                        label-width="auto" require-mark-placement="right-hanging" :size="size" :rules="fastRules">
                        <n-grid :key="model.selectedRecordType" cols="1 s:2" x-gap="12" responsive="screen">
                            ...
                            <n-grid-item>
                                <n-form-item label="隧道" :required="true" required-message="请选择隧道">
                                    <n-skeleton v-if="loadingSelectedTunnel" height="34px" width="100%"
                                        style="border-radius: 3px;" />
                                    <n-select v-else placeholder="请选择隧道" :options="tunnelOptions"
                                        v-model:value="fastModel.selectedTunnel" @update:value="handleTunnelChange" />
                                </n-form-item>
                            </n-grid-item>
                        </n-grid>
                        <div style="display: flex; justify-content: flex-end">
    ...
    <n-modal v-model:show="resetDomainNameModal">
        <n-card style="width: 600px" size="small" title="修改解析">
            <n-form style="margin-top: 10px;" :model="model" ref="formRef" label-placement="left" label-width="auto"
                require-mark-placement="right-hanging" :size="size" :rules="rules">
                <n-grid cols="1 s:2" x-gap="12" responsive="screen">
                    <n-grid-item>
                        <n-form-item label="域名" path="selectValue" :required="true" required-message="请选择域名">
                            <n-select placeholder="请选择域名" disabled :options="domainNameOptions"
                                v-model:value="model.selectedDomain" />
                        </n-form-item>
                    </n-grid-item>
                    <n-grid-item>
                        <n-form-item label="类型" :required="true" required-message="请选择记录类型">
                            <n-select placeholder="请选择记录类型" disabled :options="recordTypeOptions"
                                v-model:value="model.selectedRecordType" />
                        </n-form-item>
                    </n-grid-item>
                    <n-grid-item>
                ...
                <div style="display: flex; justify-content: flex-end">
                    <n-button round type="primary" :loading="determineLoading" @click="resetDomainName"
                        :disabled="model.target === null">
                        确定
                    </n-button>
                </div>
...
const getTooltipMessage = (record: string, domain: string): string => {
    ...
    const message = Object.keys(specialRecordMessages)
        .find(key => record.includes(key));
...
watch(domainData, (newDomainData) => {
